...
public class PublicacionLibro extends javax.swing.JFrame {
    ...
    private void BotonVenderMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_BotonVenderMouseClicked
        if (!TituloLibro.getText().isEmpty() && !AutorLibro.getText().isEmpty() && !PrecioLibro.getText().isEmpty()) {
            try {
                // TODO add your handling code here:
                if (vinculo.insertLibro(TituloLibro.getText(), AutorLibro.getText(), DescripcionLibro.getText(), 0, Double.parseDouble(PrecioLibro.getText()), UsuarioAux.getUsername())) {
                    JOptionPane.showMessageDialog(rootPane, "Libro subido con exito");
                } else {
                    JOptionPane.showMessageDialog(rootPane, "Error al registrar");
                }
            } catch (RemoteException ex) {
                Logger.getLogger(PublicacionLibro.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
